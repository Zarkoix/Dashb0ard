  ...
  start: function (server) {
    ...
    wss.on('connection', function connection(ws, req) {
      ...
      ws.on('message', function incoming(message) {
        ...
        switch (message.type) {
          case 'auth':
            jwt.verifyToken(message.token, secret).then(info => {
              debug('Auth succeeded for websocket, username: %s', info.username);
      ...
      ws.on('close', function close() {
        if (ws.username) {
          ...
          debug(map[ws.username].length + ' client(s) connected for ' + ws.username);
        }
      });
    });
